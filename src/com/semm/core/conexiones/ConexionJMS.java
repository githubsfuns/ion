...
public class ConexionJMS extends Conexion implements MessageListener{
   
   transient  Logger log = Logger.getLogger(ConexionJMS.class.getName()); 
	...
	public void onMessage(Message jms) {
		...
		if (jms instanceof ObjectMessage) {
			...
			try {
				...
				} else if (ob instanceof ExtendedTransData) {
					 mjms = (UserTransData)ob;
				} else {
					UserTransData utd = (UserTransData)ob;
					ExtendedTransData etd = new ExtendedTransData(utd);
					etd.setJmsId(om.getJMSCorrelationID());
					mjms = etd;
            	...
            	String datarr = "";
            	for(String data : mjms.getData()){
            		datarr += data + "|";
            	}      		
            	log.info( mjms.getCliente()+"|"+ mjms.recips.get(0)+"|"+datarr +mjms.getServicio() );
